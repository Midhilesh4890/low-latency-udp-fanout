#!/usr/bin/env python3
"""Build compact Session 4 tables from ignored raw benchmark results."""
...
RAW = ROOT / "benchmark" / "results" / "session4"
OUT = ROOT / "reports" / "session4"
...
def field_map(path):
    ...
    for key, value in re.findall(r"([A-Za-z0-9_]+)=([-+0-9.]+)", read_text(path)):
        try:
            ...
            # Addresses and other dotted tokens are not numeric counters.
            continue
    return values
...
def fanout_rows():
    return [
        {
            "run": "session4/fanout/control1_275k",
            "receivers": 1,
        ...
        {
            "run": "session4/fanout/fanout2_275k",
            "receivers": 2,
            "source_rate_msg_s": 275000,
        ...
        {
            "run": "session3/session3_fanout3_250k_3m",
            "receivers": 3,
        ...
        {
            "run": "session4/fanout/fanout3_275k",
            "receivers": 3,
            "source_rate_msg_s": 275000,
        ...
        {
            "run": "session4/fanout/control1_825k",
            "receivers": 1,
